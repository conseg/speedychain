...
def isBlockValid(block):
    ...
    lastBlkHash = CryptoFunctions.calculateHash(
        lastBlk.index, lastBlk.previousHash, lastBlk.timestamp, lastBlk.publicKey, lastBlk.nonce)
...
class R2ac(object):
        ...
        if (blk != False and blk.index > 0):
            ...
            if (devAESKey != False):
                ...
                if isSigned:
                    ...
                    t2 = time.time()
                    logger.info("==2=>time to add transaction in a block = " +
    ...
    def updateBlockLedger(self, pubKey, transaction):
        ...
        t2 = time.time()
        logger.info("==3=>time to update transaction received = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
        return "done"
    ...
    def updateIOTBlockLedger(self, iotBlock, gwName):
        ...
        logger.info("==4=>time to add new block in peers = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
    ...
    def addBlock(self, devPubKey):
        ...
        t3 = time.time()
        logger.info("==1=>time to generate key = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
        logger.info("==8=>time to add block (perform consensus and update all peers) = " +
    ...
    def calcMerkleTree(self, blockToCalculate):
        ...
        mt.make_tree()
        t2 = time.time()
        logger.info("==5=>time to generate merkle tree (size = " +
    ...
    def electNewOrchestrator(self):
        ...
        t2 = time.time()
        logger.info("==7=>time to execute new election block consensus = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
    ...
    def runPBFT(self):
        ...
        t2 = time.time()
        logger.info("==6=>time to execute PBFT block consensus = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
    ...
    def rundBFT(self):
        ...
        PBFTConsensus(blk, gwPub, devPubKey)
        t2 = time.time()
        logger.info("==6=>time to execute BFT block consensus = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
    ...
    def runPoW(self):
        ...
        if (PoWConsensus(blk, gwPub, devPubKey)):
            t2 = time.time()
            logger.info("==6=>time to execute PoW block consensus = " +
                        '{0:.12f}'.format((t2 - t1) * 1000))
    ...
    def runNoConsesus(self):
        ...
        sendBlockToPeers(newBlock)
        t2 = time.time()
        logger.info("==6=>time to execute block consensus (not achieved) = " +
                    '{0:.12f}'.format((t2 - t1) * 1000))
...
def commitBlockPBFT(newBlock, generatorGwPub, generatorDevicePub, alivePeers):
    ...
    while (pbftFinished and i < 20):
        pbftAchieved = handlePBFT(
            newBlock, generatorGwPub, generatorGwPub, alivePeers)
        if(not pbftAchieved):
            oldId = newBlock.index
            # logger.info("PBFT not achieve, Recreating block="+ str(ChainFunctions.getBlockchainSize()))
            newBlock = ChainFunctions.createNewBlock(
                generatorDevicePub, gwPvt, consensus)
